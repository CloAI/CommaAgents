 ...
 *
 * @param options - Configuration options for the session store.
 *
 * @example
 * ```ts
...
export function createSessionStore(
  ...
  function findExistingSessionForCwd(
    ...
    async recordRun(
      ...
      return runSerialized(sessionId, async () => {
        ...
        const existingIndex = session.runs.findIndex(
          (entry) => entry.runId === summary.runId,
        );
        const nextRuns =
          existingIndex === -1
            ? [...session.runs, summary]
            : session.runs.map((entry, index) =>
