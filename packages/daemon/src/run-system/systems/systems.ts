...
export async function invokeOnRunPrepare(
  systems: readonly DaemonSystem[],
  runContext: SystemRunContext,
  logger: Logger,
): Promise<void> {
  for (const system of systems) {
    if (system.onRunPrepare) {
      try {
        await system.onRunPrepare(runContext);
      } catch (error) {
        logger.error(
          `System ${system.name} onRunPrepare failed: ${error instanceof Error ? error.message : String(error)}`,
        );
        throw error;
      }
    }
