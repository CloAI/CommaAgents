...
describe("RequestAuthMessage", () => {
  test("parses valid message without envVar", () => {
    const msg = { type: "request_auth", ts, providerId: "openai", runId: "run-1" };
    expect(RequestAuthMessage.parse(msg)).toEqual(msg);
  });

  test("parses valid message with envVar", () => {
    const msg = {
      type: "request_auth",
      ts,
      providerId: "openai",
    ...
    expect(RequestAuthMessage.parse(msg).envVar).toBe("OPENAI_API_KEY");
  });

  test("rejects missing providerId", () => {
    expect(RequestAuthMessage.safeParse({ type: "request_auth", ts, runId: "run-1" }).success).toBe(
      false,
    );
  });
  ...
  test("rejects missing runId", () => {
    expect(
      RequestAuthMessage.safeParse({ type: "request_auth", ts, providerId: "openai" }).success,
    ).toBe(false);
  });
});
...
describe("DaemonMessage union", () => {
  ...
  test("routes request_auth correctly", () => {
    const result = DaemonMessage.parse({
      type: "request_auth",
      ts,
      providerId: "p",
    ...
    expect(result.type).toBe("request_auth");
  });
