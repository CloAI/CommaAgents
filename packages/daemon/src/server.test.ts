...
/** Mock CredentialStore that returns API key credentials for any provider. */
...
// Strategy with a user agent (blocks for input)
...
describe("user_input / provide_auth error cases", () => {
  ...
  it("provide_auth for unknown provider → error response", async () => {
    const daemon = await startDaemon();
    const client = await connectClient(daemon);
    ...
    client.send({
      ...
      credential: { type: "api", key: "sk-test" },
      scope: "$global",
      persist: false,
      requestId: "auth-1",
    });
    ...
    const err: any = await client.waitForType("error");
    expect(err.code).toBe("NO_PENDING_AUTH");
    expect(err.requestId).toBe("auth-1");
    client.close();
  });
});
