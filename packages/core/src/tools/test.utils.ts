...
export function makeToolContext(
  overrides?: Partial<Record<string, unknown>> & { guard?: Guard; sandbox?: unknown },
): ToolContext {
  ...
  } else if (overrides?.sandbox && typeof (overrides.sandbox as Record<string, unknown>)?.guardFor === "function") {
    guard = (overrides.sandbox as Record<string, (name: string) => Guard>).guardFor("test-tool");
