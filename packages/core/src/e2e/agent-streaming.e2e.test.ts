...
describe("E2E: Agent Streaming", () => {
  ...
  describe("streaming with tool calls", () => {
    it("should yield tool-call and tool-result events during stream", async () => {
      ...
      // NOTE: There is a bug in base-agent.ts line 242 — it reads `part.result`
      // but the AI SDK v3 fullStream uses `part.output` for tool-result parts.
      // This means toolResultEvent.output is currently undefined.
      // When the bug is fixed, this test should verify the actual tool output.
      ...
      // TODO: Uncomment when base-agent.ts bug is fixed (part.result → part.output):
