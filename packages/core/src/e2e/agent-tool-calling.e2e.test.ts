...
import { defineTool } from "../tools/define/define-tool";
...
/**
 * Small settle helper — gives the event loop a chance to flush.
 */
...
describe("E2E: Agent Tool Calling", () => {
  ...
  describe("single tool call round-trip", () => {
    ...
    it("should record tool calls in step results", async () => {
      ...
      const toolStep = result.steps.find((s: any) => s.toolCalls && s.toolCalls.length > 0);
    ...
    it("should handle model calling multiple tools across rounds", async () => {
      ...
      const model = createToolCallingMockModel({
        rounds: [
          ...
          {
            toolCalls: [{ id: "c2", name: "recorder", args: { input: "logging weather data" } }],
          },
