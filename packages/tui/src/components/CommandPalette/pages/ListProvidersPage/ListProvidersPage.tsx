...
import { useDaemonContext } from "../../../../hooks/useDaemon";
 ...
 * is registered.
 */
export function ListProvidersPage({ focusId }: { readonly focusId: string }): React.ReactElement {
  const debug = useDebugRender("ListProvidersPage", {});
  const { send, on } = useDaemonContext();
          ...
          renderItem={(p, isSelected) => (
            ...
            >
              ...
              <Box width={14} flexShrink={0}>
                <Text color={p.authStatus === "configured" ? tokens.colors.success : tokens.colors.muted}>
