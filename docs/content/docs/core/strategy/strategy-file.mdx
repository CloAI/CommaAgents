...
`providerOptions` passes provider-specific features to the AI SDK (e.g. Anthropic extended thinking, OpenAI reasoning effort). `modelOptions` passes provider-agnostic generation parameters (`temperature`, `maxOutputTokens`, `topP`, `seed`, etc.). `outputSchema` accepts a JSON Schema object for structured output. See [createAgent](/docs/core/agents/create-agent) for details.
