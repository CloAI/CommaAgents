---
title: Skills
description: Reusable instruction bundles that agents load on demand via the load_skill tool.
---
...
The framework ships no built-in skills. You author them; they are discovered automatically from two locations and surfaced to every agent's system prompt as a compact one-line summary. The agent decides when to load the full content by calling the [`load_skill`](/docs/core/tools/built-in/load_skill) tool.
...
Skills are discovered from two roots. Both are scanned on every strategy run; project skills override global skills with the same `name`.

| Location | Path | Purpose |
|---|---|---|
| Global | `<configRoot>/comma-agents/skills/` | Personal skills available across all projects. |
| Project | `<workspaceRoot>/.comma/skills/` | Project-specific skills checked into the repo. |
...
Each direct subdirectory containing a `SKILL.md` becomes a skill. Deeper nesting is ignored. Subdirectories without a `SKILL.md` are silently skipped, so you can keep supporting files (scripts, examples) alongside the skill.
...
The strategy loader builds a `## Available Skills` block from the registry and prepends it to every agent's system prompt. Agents call the [`load_skill`](/docs/core/tools/built-in/load_skill) tool to fetch the body of a skill they recognise as relevant.
