...
`loadFlow` validates `config` with the schema provided by the registration. Unknown types and invalid configuration fail during loading. See [Custom Flows](/docs/core/flows/custom-flow) for registration examples.
