...
**`DEFAULT_SANDBOX_CONFIG`** — jailed to `process.cwd()`, absolute paths rejected, standard forbidden globs (`.git/**`, `.env*`, keys, certs), reads and writes allowed within the boundary. Recommended for most projects.
...
**`DEFAULT_DAEMON_SANDBOX_CONFIG`** — jailed to cwd, absolute paths allowed (daemon passes absolute paths), in-cwd paths auto-allowed via `allow: ["**"]`, anything outside cwd triggers `"ask"` via the permission bridge. Leaves `cwd` unset so the daemon supplies it at runtime.
