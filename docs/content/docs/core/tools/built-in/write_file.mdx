...
If something else has changed the file in between, `write_file` returns `stale_file` with the message *"Re-read the file to obtain the current sha256 and re-apply your edit."* See the [Stale file protocol](/docs/core/tools/stale-file-protocol) page for the full lifecycle.
...
- [read_file](/docs/core/tools/built-in/read_file) — produces the `sha256` you pass back here.
- [edit_file](/docs/core/tools/built-in/edit_file) — change only a region of the file instead of rewriting it.
- [apply_patch](/docs/core/tools/built-in/apply_patch) — apply a multi-file diff atomically.
