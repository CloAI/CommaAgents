...
No body. The pre-image is captured for rollback and the file is moved to the workspace trash on commit (same mechanism as [delete_file](/docs/core/tools/built-in/delete_file)).
...
- [apply_patch](/docs/core/tools/built-in/apply_patch) — the tool that consumes this envelope.
- [Stale file protocol](/docs/core/tools/stale-file-protocol) — `expectedSha256ByPath` semantics.
