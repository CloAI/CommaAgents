...
`resolveCredentialsPath()` returns the platform-aware default path:

| Platform | Path |
|---|---|
| macOS | `~/Library/Application Support/comma-agents/credentials.json` |
| Linux | `$XDG_DATA_HOME/comma-agents/credentials.json` (default `~/.local/share/comma-agents/credentials.json`) |
| Windows | `%LOCALAPPDATA%/comma-agents/credentials.json` |
