...
Conversation context is per-agent state. Each agent created with `createAgent` owns an internal context that stores canonical conversation records and projects those records into AI SDK messages for the next model call.
